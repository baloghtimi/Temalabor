/**
 * generated by Xtext 2.10.0
 */
package org.xtext.example.mydsl.myDsl;

import java.util.Arrays;
import java.util.Collections;
import java.util.List;

import org.eclipse.emf.common.util.Enumerator;

/**
 * <!-- begin-user-doc -->
 * A representation of the literals of the enumeration '<em><b>Card Type</b></em>',
 * and utility methods for working with them.
 * <!-- end-user-doc -->
 * @see org.xtext.example.mydsl.myDsl.MyDslPackage#getCardType()
 * @model
 * @generated
 */
public enum CardType implements Enumerator
{
  /**
   * The '<em><b>Blue Color</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #BLUE_COLOR_VALUE
   * @generated
   * @ordered
   */
  BLUE_COLOR(0, "BlueColor", "BlueCard"),

  /**
   * The '<em><b>Green Color</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #GREEN_COLOR_VALUE
   * @generated
   * @ordered
   */
  GREEN_COLOR(1, "GreenColor", "GreenCard"),

  /**
   * The '<em><b>Orange Color</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #ORANGE_COLOR_VALUE
   * @generated
   * @ordered
   */
  ORANGE_COLOR(2, "OrangeColor", "OrangeCard"),

  /**
   * The '<em><b>Yellow Color</b></em>' literal object.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @see #YELLOW_COLOR_VALUE
   * @generated
   * @ordered
   */
  YELLOW_COLOR(3, "YellowColor", "YellowCard");

  /**
   * The '<em><b>Blue Color</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of '<em><b>Blue Color</b></em>' literal object isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @see #BLUE_COLOR
   * @model name="BlueColor" literal="BlueCard"
   * @generated
   * @ordered
   */
  public static final int BLUE_COLOR_VALUE = 0;

  /**
   * The '<em><b>Green Color</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of '<em><b>Green Color</b></em>' literal object isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @see #GREEN_COLOR
   * @model name="GreenColor" literal="GreenCard"
   * @generated
   * @ordered
   */
  public static final int GREEN_COLOR_VALUE = 1;

  /**
   * The '<em><b>Orange Color</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of '<em><b>Orange Color</b></em>' literal object isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @see #ORANGE_COLOR
   * @model name="OrangeColor" literal="OrangeCard"
   * @generated
   * @ordered
   */
  public static final int ORANGE_COLOR_VALUE = 2;

  /**
   * The '<em><b>Yellow Color</b></em>' literal value.
   * <!-- begin-user-doc -->
   * <p>
   * If the meaning of '<em><b>Yellow Color</b></em>' literal object isn't clear,
   * there really should be more of a description here...
   * </p>
   * <!-- end-user-doc -->
   * @see #YELLOW_COLOR
   * @model name="YellowColor" literal="YellowCard"
   * @generated
   * @ordered
   */
  public static final int YELLOW_COLOR_VALUE = 3;

  /**
   * An array of all the '<em><b>Card Type</b></em>' enumerators.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private static final CardType[] VALUES_ARRAY =
    new CardType[]
    {
      BLUE_COLOR,
      GREEN_COLOR,
      ORANGE_COLOR,
      YELLOW_COLOR,
    };

  /**
   * A public read-only list of all the '<em><b>Card Type</b></em>' enumerators.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public static final List<CardType> VALUES = Collections.unmodifiableList(Arrays.asList(VALUES_ARRAY));

  /**
   * Returns the '<em><b>Card Type</b></em>' literal with the specified literal value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param literal the literal.
   * @return the matching enumerator or <code>null</code>.
   * @generated
   */
  public static CardType get(String literal)
  {
    for (int i = 0; i < VALUES_ARRAY.length; ++i)
    {
      CardType result = VALUES_ARRAY[i];
      if (result.toString().equals(literal))
      {
        return result;
      }
    }
    return null;
  }

  /**
   * Returns the '<em><b>Card Type</b></em>' literal with the specified name.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param name the name.
   * @return the matching enumerator or <code>null</code>.
   * @generated
   */
  public static CardType getByName(String name)
  {
    for (int i = 0; i < VALUES_ARRAY.length; ++i)
    {
      CardType result = VALUES_ARRAY[i];
      if (result.getName().equals(name))
      {
        return result;
      }
    }
    return null;
  }

  /**
   * Returns the '<em><b>Card Type</b></em>' literal with the specified integer value.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the integer value.
   * @return the matching enumerator or <code>null</code>.
   * @generated
   */
  public static CardType get(int value)
  {
    switch (value)
    {
      case BLUE_COLOR_VALUE: return BLUE_COLOR;
      case GREEN_COLOR_VALUE: return GREEN_COLOR;
      case ORANGE_COLOR_VALUE: return ORANGE_COLOR;
      case YELLOW_COLOR_VALUE: return YELLOW_COLOR;
    }
    return null;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private final int value;

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private final String name;

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private final String literal;

  /**
   * Only this class can construct instances.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  private CardType(int value, String name, String literal)
  {
    this.value = value;
    this.name = name;
    this.literal = literal;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public int getValue()
  {
    return value;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public String getName()
  {
    return name;
  }

  /**
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  public String getLiteral()
  {
    return literal;
  }

  /**
   * Returns the literal value of the enumerator, which is its string representation.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @generated
   */
  @Override
  public String toString()
  {
    return literal;
  }
  
} //CardType
